...
These are documented here so the next slice doesn't re-derive the decision. The user has explicitly flagged that they want to come back to slices D and E once this slice lands:

- **Slice D (queued): Adopt rmcp 1.x's `auth` feature** to replace `crates/zotero-mcp/src/oauth.rs` (~700 lines). Saves a substantial maintenance burden, but uncertain whether rmcp's OAuth flavour matches Zotero's specific requirements (PKCE S256, custom issuer config, RFC 9728 protected-resource metadata, file-backed token store with constant-time digest comparison, refresh-token replay revocation). Requires its own research spike before any code lands.

- **Slice E (queued): Adopt rmcp 1.x's `transport-streamable-http-server` feature** to replace `crates/zotero-mcp/src/http_transport.rs` (~150 lines). rmcp 0.1 used SSE; rmcp 1.x uses the new streamable-HTTP wire protocol. Different on-the-wire shape — existing MCP clients may need their transport layer updated. Requires testing against the actual MCP client (Claude Desktop) before committing.
