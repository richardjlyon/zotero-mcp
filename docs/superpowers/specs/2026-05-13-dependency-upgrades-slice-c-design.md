...
- **Slice E (queued, premise NOT yet verified): Adopt rmcp 1.x's `transport-streamable-http-server` feature** to replace `crates/zotero-mcp/src/http_transport.rs` (~150 lines). rmcp 0.1 used SSE; rmcp 1.x uses the new streamable-HTTP wire protocol. Different on-the-wire shape — existing MCP clients may need their transport layer updated. **Before any further work**, verify that rmcp 1.7 actually ships server-side `transport-streamable-http-server` scaffolding (this was Slice A's claim but Slice D's lesson is that those claims need direct source verification). If it does, the slice also needs testing against the actual MCP client (Claude Desktop) before committing because the protocol shape change is client-visible.
