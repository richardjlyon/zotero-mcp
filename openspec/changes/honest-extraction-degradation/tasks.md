...
- [ ] 1.1 `Error::LayoutRouteUnavailable { path, endpoint }` beside `PdfDocumentTooLarge`
...
- [ ] 1.2 Gate in `extract_core` (`core/pdf.rs:1370`): configured-and-unhealthy without
...
- [ ] 1.3 Tests for all five rows of the D1 table, including `plain=true` succeeding
  without the opt-in and no-route-configured succeeding without it.

## 2. The opt-in (D3)

- [ ] 2.1 `allow_degraded: bool` (default false) on `PdfTextArgs` and `FirstPagesArgs`,
  threaded to `extract_core`. Update tool descriptions to state the `plain` /
  `allow_degraded` distinction in one sentence.
- [ ] 2.2 Wire-shape and tool-surface tests updated for the new argument.
- [ ] 2.3 Test: with the opt-in set on an unhealthy configured route, the flat chain runs,
  is labelled, and reports `complete: false` exactly as before.

## 3. The text stays clean (D4)

- [ ] 3.1 Test: no degraded path appends any marker, banner or warning to `r.text` — the
  text contains only document content, on every posture.
- [ ] 3.2 Confirm the CLI's stderr header remains the degradation channel and already
  carries route and completeness (`src/main.rs`), and that stdout is untouched.

## 4. CLI posture (D5)

- [ ] 4.1 `pdf-text --allow-degraded`, passed through; without it a
  `LayoutRouteUnavailable` prints the error to stderr and exits non-zero.
- [ ] 4.2 Window walk aborts on the first refused window rather than streaming a stream
  with an unmarked hole. Test the abort and the exit code.

## 5. Store interaction (D6)

- [ ] 5.1 A store hit bypasses the gate entirely (stored content is layout-faithful by
...
- [ ] 6.1 CHANGELOG entry marked BREAKING, naming the argument, the new error, and the
  two known consumers.
- [ ] 6.2 Migration note in `docs/`: what changes for a caller on a host with a configured
  layout route, and the one-argument escape.
- [ ] 6.3 Minor version bump; ship separately from the persistence change.

## 7. Regression suite

- [ ] 7.1 Table-fidelity regression over the committed fixtures from the sister change:
  assert table count and structure, never character count.
- [ ] 7.2 Test that a character-count-only comparison would fail to distinguish the routes
  — the guard against the 3,496-vs-3,299 trap.
- [ ] 7.3 `cargo test` green on a host with no layout service; layout-dependent cases skip
  loudly.

## 8. Verification

- [ ] 8.1 With the service stopped, confirm `get_pdf_text` refuses by default and succeeds
  with the opt-in; with it warm, confirm markdown and tables.
