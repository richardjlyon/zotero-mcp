...
async fn windowed_docling_anchors_carry_true_page_numbers_live() {
    ...
    assert!(
        lower.contains("badger") && lower.contains("capybara"),
        "got: {:?}",
        r.text
    );
    assert!(
        !lower.contains("albatross"),
        "page-1 text must not leak into the window"
    );
    assert_eq!(
        r.completeness.total_pages, 3,
        "total_pages is the whole document"
    );
    assert_eq!(
        r.completeness.pages, 2,
        "pages describes only the 2-page window"
    );
}
