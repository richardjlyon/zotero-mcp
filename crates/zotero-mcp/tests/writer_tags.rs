...
use zotero_mcp::core::writer::tags::{add_tags, remove_tags, add_to_collection, remove_from_collection};
...
async fn remove_tags_filters_out_specified() {
    ...
    let api = LocalApi::new(server.uri(), 93338).unwrap();
    remove_tags(&api, "JGF2UTMW", &["remove-me".into()]).await.unwrap();
}
...
async fn remove_from_collection_round_trips() {
    ...
    let api = LocalApi::new(server.uri(), 93338).unwrap();
    remove_from_collection(&api, "JGF2UTMW", "COLL1").await.unwrap();
}
