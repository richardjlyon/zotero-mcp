...
use zotero_mcp::core::citations::{format_bibliography, format_citation};
...
async fn formats_single_citation_as_bib() {
    ...
    Mock::given(method("GET"))
        ...
        .respond_with(
            ResponseTemplate::new(200).set_body_string("<div>Rabkin, Y. (2016). ...</div>"),
        )
        .mount(&server)
        .await;

    let api = LocalApi::new(server.uri(), 93338).unwrap();
    let s = format_citation(&api, "JGF2UTMW", "apa", "bib")
        .await
        .unwrap();
